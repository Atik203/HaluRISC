% HaluRISC journal manuscript — Elsevier CAS single-column format.
%
% Structure mirrors Journal_Paper/cas-sc-template.tex: six sections
% (Introduction, Conventional Method, Proposed Method, Experiment, Results,
% Conclusion) followed by Declarations and the bibliography.
%
% Numbers in this manuscript come from the frozen Version B artifacts
% (docs/manifest.frozen.json, artifacts/results/b2..b5). Do not edit numbers
% here without regenerating and re-verifying the artifact manifest.
%
% Compile from Journal_Paper/:
%   latexmk -pdf -outdir=out halurisc.tex
%
% Figures resolve through \graphicspath below. The reference database is the
% local ref.bib (all entries verified 2026-09-17).
...
\graphicspath{{../report/figures/}{../report/screenshots/}{../artifacts/figures/}}
...
% Short title
\shorttitle{HaluRISC: Calibrated Hallucination Risk Estimation}

% Short author
\shortauthors{Rahaman et~al.}

% Main title of the paper
\title [mode = title]{HaluRISC: Cross-Domain Calibrated and Explainable Hallucination Risk Estimation for Black-Box LLM Responses}
...
\begin{keywords}
Hallucination detection \sep Explainable AI \sep Probability calibration \sep Distribution shift \sep XGBoost \sep SHAP
\end{keywords}
...
Large language models are increasingly embedded in real products, from chatbots to retrieval-augmented question answering and content review tools, where a confidently wrong answer has a real cost. The models occasionally generate text that is fluent but factually unsupported, an effect known as \emph{hallucination}, and detecting it is a first-class safety problem for anyone deploying these systems. One dominant strategy asks a strong LLM to judge its own output, and SelfCheckGPT implements this by sampling several responses and measuring their consistency without external resources \cite{selfcheck}. Small specialized evaluators such as Luna \cite{luna} make the judge approach cheaper, yet the pattern remains slow because every response needs a full forward pass. Neural classifiers over frozen encoders \cite{ieeeTai} are more accurate, but they require the model weights, GPU inference, and their decisions resist interpretation. This paper takes a different route. HaluRISC treats hallucination detection as a supervised risk-prediction task over engineered evidence-consistency features, which measure how well the answer agrees with the question and the reference context in vocabulary, named entities, logical entailment, numbers, hedging, and semantics. A tuned XGBoost classifier consumes these features and outputs a calibrated probability, and SHAP explains every verdict with the feature contributions that drove it, so the system never inspects the LLM, runs in tens of milliseconds on a CPU, and costs essentially nothing per prediction. The main contributions are a reproducible feature-extraction pipeline with 26 features in seven groups, including bidirectional NLI entailment and contradiction signals, and a rigorous comparison of heuristic, logistic regression, random forest, and XGBoost baselines under 5-fold cross-validation with a three-seed protocol. The study also compares Platt and isotonic calibration fit strictly on the validation split, evaluates zero-shot transfer to the natural RAGTruth corpus, and measures explanation reliability with perturbation analysis and feature-removal correlation. On the HaluEval QA benchmark \cite{haluEval} the model reaches F1 = 0.9846 and AUROC = 0.9979 with an expected calibration error of 0.0045, while transfer to natural RAGTruth QA \cite{ragtruth} is poor (F1 = 0.3022), and this negative result is reported openly rather than hidden. The full system is deployed as a FastAPI backend with a Next.js and assistant-ui dashboard. The rest of the paper is organized as follows. Section~\ref{sec:conventional} reviews the conventional approaches to hallucination detection, Section~\ref{sec:proposed} describes the feature pipeline, the model, and the deployed system, Section~\ref{sec:experiment} details the experimental setup, Section~\ref{sec:results} reports results on calibration, cross-domain transfer, and explanation reliability, and Section~\ref{sec:conclusion} concludes.
...
\begin{figure}[pos=htbp]
\centering
\includegraphics[width=0.62\linewidth]{hallucinated_date.png}\\
\vspace{0.3em}
\includegraphics[width=0.62\linewidth]{grounded_correct.png}
...
\caption{Reliability diagrams of the raw, Platt-calibrated, and isotonic models on the HaluEval test set.}
...
\caption{Score distributions before and after target-domain recalibration on RAGTruth QA.}
...
\caption{Calibrated score distributions on the source test set and the zero-shot corpora.}
