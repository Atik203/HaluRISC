...
\credit{Conceptualization, Data curation, Formal analysis, Investigation, Methodology, Project administration, Resources, Software, Validation, Visualization, Writing - original draft, Writing - review \& editing}
...
\begin{table}[pos=htbp]
  \centering
  \caption{Claim-level NLI aggregates (8 features). The answer is split into at most six atomic clauses, with conjunctions such as ``and'' or ``but'' broken into separate clauses, each clause is scored against its four best-matching context sentences, and the pair probabilities are aggregated.}
  \label{tab:claim}
...
Beyond the score, the system verifies the answer claim by claim. Each claim is decomposed from the answer text and checked with sentence-level NLI against one of three evidence sources, the provided context, an indexed document collection, or live web search. Verdicts are decided support-first, so any entailing sentence supports a claim and a contradiction decides only when no sentence supports it, and a relevance gate drops retrieved sentences that do not cover the claim vocabulary, so an off-topic page cannot decide a verdict. The verdicts supported, contradicted, and unsupported are rendered per claim, with the driving evidence sentence quoted for contradicted claims. Web search uses the Brave Search API with its LLM-context endpoint, and the document index combines BM25, FAISS dense retrieval, and reciprocal rank fusion with a cross-encoder reranker. When the verification score falls into an uncertain band, the claim is routed to an LLM judge that returns a verdict with a short reasoning note.
...
Figure~\ref{fig:screens} shows three analysis cases from the analyze page, each with the SHAP contributions of the raw model and the model-comparison panel. The raw EC-XGB probability is 0.999 for the hallucinated number, 0.132 for the grounded answer, and 0.008 for the one-word answer, while the deployed display calibrator reports 35.4\%, 4.8\%, and 3.4\%. The first case shows the value of the calibrator, because a raw probability near one is not an actionable risk level. The third case is a one-word answer whose strongest contributors are length features, which echoes the style sensitivity noted above.
...
\begin{figure}[pos=htbp]
  ...
  \begin{subfigure}[t]{0.62\linewidth}
    ...
    \caption{Hallucinated number: display score 0.354 (medium risk)}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.34\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=1.9in,keepaspectratio]{hallucinated_shap_model_compare.png}
    \caption{SHAP and comparison, hallucinated}
  \end{subfigure}
  \\[0.6em]
  \begin{subfigure}[t]{0.62\linewidth}
    \centering
    \includegraphics[width=\linewidth]{grounded_correct.png}
    \caption{Grounded answer: display score 0.048 (low risk)}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.34\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=1.9in,keepaspectratio]{ground_shap_model_compare.png}
    \caption{SHAP and comparison, grounded}
  \end{subfigure}
  \\[0.6em]
  \begin{subfigure}[t]{0.62\linewidth}
    \centering
    \includegraphics[width=\linewidth]{borderline.png}
    \caption{One-word answer: display score 0.034 (low risk)}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.34\linewidth}
    \centering
    \includegraphics[width=\linewidth,height=1.9in,keepaspectratio]{borderline_shap_model_compare.png}
    \caption{SHAP and comparison, one-word answer}
  \end{subfigure}
  \caption{Analyze-page cases with the raw-model SHAP contributions and the model-comparison panel. Top: a hallucinated number, where the context states 5 days per decade and the answer states 10. Middle: a grounded answer. Bottom: a one-word answer whose strongest contributors are length features. The gauge shows the calibrated display score, and the comparison panel shows the raw probability of each model.}
  \label{fig:screens}
\end{figure}
...
\begin{table}[pos=htbp]
  ...
  \begin{tabular}{|l|c|c|c|c|c|}
    ...
    \hline
    m2 + claims  & 34 & 0.9857 & 0.9984 & 0.9717 & 0.0078 \\
    \hline
    m3 EC-XGB    & 35 & 0.9855 & 0.9984 & 0.9715 & 0.0074 \\
    \hline
  \end{tabular}
\end{table}

Table~\ref{tab:shift} shows where the modification matters. Every model is applied without retraining to the RAGTruth and FaithBench corpora. The standard model flags almost every natural answer as risky, since 99.9\% of RAGTruth rows and 99.5\% of FaithBench rows cross the 0.5 threshold, which makes the decision rule useless outside the benchmark even though recall stays high. EC-XGB cuts the flag rate to 61.3\% on the RAGTruth test tasks and to 7.3\% on FaithBench, raises the AUROC from 0.475 to 0.582 and from 0.509 to 0.594, and lowers the ECE on RAGTruth from 0.635 to 0.278. The held-out RAGTruth QA task is the strictest test, since its rows never enter training. There the F1 stays flat (0.3022 against 0.3081) and the ECE improves from 0.815 to 0.743, which shows that multi-source training does not transfer a new task for free. On FaithBench the conservative regime lowers the F1 from 0.813 to 0.120. The standard model earns that F1 by flagging nearly every row, so the AUROC and the flag rate are the fair comparison at this operating point, and the F1 drop is reported as a limitation rather than hidden.
...
\begin{table}[pos=htbp]
  ...
  \begin{tabular}{|l|l|c|c|c|c|}
    ...
    \hline
    RAGTruth QA  & EC-XGB   & 0.3081 & 0.5491 & 97.4\% & 0.7433 \\
    \hline
...
Hallucination detection needs to be cheap, fast, and explainable before it is useful in practice. This study presented EC-XGB, an evidence-consistent XGBoost risk predictor that combines 26 engineered evidence-consistency features with eight claim-level NLI aggregates, log-scaled length features under monotone constraints, multi-source training, calibrated probabilities, and SHAP explanations. On HaluEval QA it reaches an F1 of 0.9857 and an AUROC of 0.9985, on par with the standard tuned XGBoost on a saturated benchmark, and it makes fewer false positives at the same operating point. The gain appears under domain shift, where the standard model flags 99.9\% of natural RAGTruth answers as risky and EC-XGB flags 61.3\%, with the AUROC rising from 0.475 to 0.573 and the ECE falling from 0.635 to 0.278. The deployed display calibrator cuts the RAGTruth QA ECE from 0.726 to 0.126. One analysis takes 61.8\,ms at the median and costs about \$0.001 per 1{,}000 predictions, roughly 100 times less than the GPT 5.6 Luna judge that it outperforms on the same 200 samples.
