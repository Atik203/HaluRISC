...
\begin{table}[pos=htbp]
  ...
  \begin{tabular}{|l|r|r|r|r|r|r|r|r|}
    ...
    \hline
    XGBoost (ours)          & 0.993      & 0.976      & 0.985       & 0.998          & 0.998           & 0.970        & 0.004        & 0.012          \\
    \hline
  \end{tabular}
\end{table}

The comparison so far uses aggregate metrics. Table~\ref{tab:cases} shows how the same evidence produces different risk scores at the level of single answers. Cases 1 and 2 share one question and one context. Case 1 is a grounded answer span, and case 2 is a hallucinated sentence that reuses words from the context. The overlap heuristic separates the pair poorly (0.0000 against 0.0769), because the hallucinated sentence copies context words, and the NLI-only model fails in the same way (0.3928 against 0.3950). The answer-only TF-IDF model flags the hallucinated case at 0.9121 but also puts the grounded span at 0.7490. Logistic regression, random forest, and XGBoost separate the pair, with 0.4975 against 0.9990, 0.5593 against 0.9924, and 0.4541 against 0.9996. Case 3 is a grounded one-word answer. The heuristic scores it at 1.0000, logistic regression at 0.7109, the random forest at 0.7634, and the raw XGBoost at 0.9135, so four models flag a correct answer as risky. The deployed calibrator maps these raw probabilities to 0.0000 and removes the false alarm, which is why the system displays the calibrated score and keeps the raw model score as a secondary signal.

\begin{table}[pos=htbp]
  \centering
  \caption{Risk scores for three HaluEval test cases, seed 42. Cases 1 and 2 share one question and one context: case 1 is a grounded answer span, and case 2 is a hallucinated sentence that reuses context words. Case 3 is a grounded one-word answer. The last row is the deployed calibrated score. A low score is correct for cases 1 and 3, and a high score is correct for case 2. Values are read from \texttt{b2\_case\_scores.csv}.}
  \label{tab:cases}
  \footnotesize
  \setlength{\tabcolsep}{6pt}
  \begin{tabular}{|l|r|r|r|}
    \hline
    \textbf{Model} & \multicolumn{3}{c|}{\textbf{Risk score}} \\
    \hline
                    & \textbf{Case 1} & \textbf{Case 2} & \textbf{Case 3} \\
    \hline
    Overlap heuristic     & 0.0000          & 0.0769          & 1.0000          \\
    \hline
    Logistic regression   & 0.4975          & 0.9990          & 0.7109          \\
    \hline
    Random forest         & 0.5593          & 0.9924          & 0.7634          \\
    \hline
    NLI-only              & 0.3928          & 0.3950          & 0.3058          \\
    \hline
    TF-IDF (answer-only)  & 0.7490          & 0.9121          & 0.1694          \\
    \hline
    XGBoost (raw)         & 0.4541          & 0.9996          & 0.9135          \\
    \hline
    Deployed (calibrated) & 0.0000          & 0.4265          & 0.0000          \\
    \hline
  \end{tabular}
\end{table}
