...
- **FastAPI Python Backend Environment:**
  ...
  - **Keys:** `FASTAPI_HOST`, `FASTAPI_PORT`, `OPENAI_API_KEY` (for `/judge`), `OPENAI_MODEL`, `BRAVE_SEARCH_API_KEY` (Tier 3 web search via Brave LLM Context + Web Search, server-side only; Tavily remains the legacy fallback via `TAVILY_API_KEY`), `BRAVE_ANSWERS_API_KEY` (optional Brave Answers plan key for `POST /answer`; generated replies stay OUT of the verification evidence chain), `HALU_WEB_SEARCH_PROVIDER` (`auto`|`brave`|`tavily`), `HALU_BRAVE_MIN_INTERVAL`/`HALU_BRAVE_CONTEXT_TOKENS`, `HALU_API_DEVICE` (`cuda`|`cpu`, default `cpu`; `cuda` auto-falls back to CPU if torch has no CUDA, models load fp16 on CUDA), `HALU_API_PRELOAD` (default `1`; `0` skips the startup preload of heavy spaCy/NLI/SBERT models), `HALU_XGB_DEVICE` (`cuda`|`cpu`|`auto`; set `cpu` in Colab so saved XGBoost models are portable across platforms — CUDA-trained boosters do not unserialize cross-platform), `HALU_JUDGE_CONF_LOW`/`HALU_JUDGE_CONF_HIGH`/`HALU_JUDGE_MAX_CLAIMS`, `HALU_RATE_*` (Tier 4 judge routing and rate limits).
...
4. **API contract:** frontend consumes `POST /api/ml/predict` → `{risk_score, calibrated_score, legacy_score, label, thresholds, latency_ms, model_version, feature_version, warning, features}` and `POST /api/ml/explain` → `{top_features[], base_value}`. The deployed model is EC-XGB (Evidence-Consistent XGBoost, `b6-ec-xgb-v1.0`): `risk_score` is its raw probability over 35 features (26 base + 8 claim-level NLI aggregates over atomic clauses + natural-response source indicator), `calibrated_score` is its Platt display score fitted on the RAGTruth QA calibration split (adjusted by per-claim verdicts in `/verify` when claims exist), and `legacy_score` is the standard B2 model's HaluEval-Platt score. `POST /api/ml/predict/compare` adds `ec_xgb` beside the standard baselines. Keep field names stable.
...
4. **References:** `Journal_Paper/ref.bib` (30 entries) is verified against real publications. Do not add references without web verification. The manuscript follows the CAS sample and names that chapter `Conventional Method`.
